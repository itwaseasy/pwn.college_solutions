...
class CImgFile:
    ...
    def generate_render_sprite_instruction(self, sprite, rgb, x, y):
        self.remaining_directives +=1
    ...
    def generate_borders(self):
        horizontal = self.create_sprite(
                self.Rectangle(x=1, y=0, width=self.width-2, height=1))

        vertical = self.create_sprite(
                self.Rectangle(x=0, y=0, width=1, height=self.height))

        self.generate_load_sprite_instruction(horizontal)
        self.generate_load_sprite_instruction(vertical)

        self.generate_render_sprite_instruction(horizontal, (b'\xff',b'\xff',b'\xff'), 1, 0)
        self.generate_render_sprite_instruction(horizontal, (b'\xff',b'\xff',b'\xff'), 1, self.height-1)

        self.generate_render_sprite_instruction(vertical, (b'\xff',b'\xff',b'\xff'), 0, 0)
        self.generate_render_sprite_instruction(vertical, (b'\xff',b'\xff',b'\xff'), self.width-1, 0)
